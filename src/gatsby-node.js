...
export const sourceNodes = async (
  ...
  const tildaPageNodes = getNodes()
    .filter((n) => n.internal.owner === OWNER && n.internal.type === PAGE_TYPE);

  const tildaAssetsNodes = getNodes()
    .filter((n) => n.internal.owner === OWNER && n.internal.type === PAGE_ASSET_TYPE);

  const deletedPageNodes = tildaPageNodes
    .filter((n) => !pages.find((p) => p.id === n.pageId));
  const deletedAssets = tildaAssetsNodes
    .filter((n) => deletedPageNodes.find((pN) => pN.pageId === n.pageId));
  const newPages = pages
    .filter((p) => !tildaPageNodes.find((n) => p.id === n.pageId));
  const updatedPages = pages
    .filter((p) => tildaPageNodes.find((n) => p.id === n.pageId && n.published !== p.published));
  const notModifiedPages = pages
    .filter((p) => tildaPageNodes.find((n) => p.id === n.pageId && n.published === p.published));
  ...
  // clear assets cache
  await Promise.all(deletedAssets.map(async (asset) => {
    const { id, url } = asset;
  ...
  console.log(`Created/Updated pages: ${[...newPages, ...updatedPages].length}`);
